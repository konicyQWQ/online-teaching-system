<template>
  <a-card class="ant-card-shadow">
      ...
      <a-table :columns="columns" :data-source="homeworkState.data" :loading="homeworkState.loading" :row-key="record => record.homework.hwId">
        ...
        <template #statistics="{ text, record }">
          {{record.statistics.totalCount}} / {{ record.statistics.submitCount }} / {{ record.statistics.totalCount - record.statistics.submitCount }}
        </template>
