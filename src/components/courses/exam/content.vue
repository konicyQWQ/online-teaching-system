<template>
  <div>
    ...
    >
      ...
      <div v-show="activeTabKey === 'base'">
        <a-descriptions bordered :column="4">
          ...
          <a-descriptions-item label="考试题目">
            <a @click="previewExam">查看考试题目</a>
          </a-descriptions-item>
        </a-descriptions>
        <a-row
          :gutter="16"
        ...
        >
          ...
          <a-col :span="8">
            <a-statistic
              title="提交人数"
              :value="state.statistics.finishedCount"
            >
              <template #suffix>
                <span> / {{ state.statistics.totalCount }}</span>
              </template>
          ...
          <a-col :span="8">
            <a-statistic
              title="平均分"
              :value="state.statistics.averageMark.toFixed(2)"
            >
              <template #suffix>
    ...
    <a-card class="ant-card-shadow" style="margin-top: 1em">
      ...
      <a-table
        :columns="columns"
        :data-source="state.userExams"
        :loading="state.loading"
      >
        <template #action="{ record }">
          <a v-if="record.userExam" @click="examScore(record)">批改</a>
          <span v-else>学生未参加考试</span>
        </template>
      </a-table>
    </a-card>
  </div>
                ...
                data: [
                  ...
                  return {
                    ...
                    value: state.userExams.reduce((acc, { userExam }) => {
                      if (index === 0)
                        return acc + (userExam && userExam.mark <= 59 ? 1 : 0)
                      else if (index === 1)
                        return (
                          acc +
                          (userExam &&
                        ...
                        return acc + (userExam && userExam.mark === 100 ? 1 : 0)
                    }, 0),
                  }
                }),
              },
            ...
            series: [
              {
                ...
                type: 'bar',
                barWidth: '50%',
                data: state.questions.map(
                  (value, index) => value.statistics.averageScore
                ),
              },
            ],
          }
